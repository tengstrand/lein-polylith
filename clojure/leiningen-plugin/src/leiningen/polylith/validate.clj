...
(defn create-component [ws-dir top-ns name]
  (let [{:keys [changed-components]} (core/info ws-dir)]
    (cond
      (is-empty? name) [false "Missing name."]
...
(defn create [ws-dir top-ns cmd name ws-ns]
  (let [{:keys [changed-components]} (core/info ws-dir)]
    (condp = cmd
      "c" (create-component ws-dir top-ns name)
